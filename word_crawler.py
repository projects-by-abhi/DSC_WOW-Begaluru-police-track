...
linkex = open('list.txt', 'r') 
alerter = open('alerts.txt', 'a+')
...
while True:
    ...
    wordex = open('words.txt', 'r')         #extracts the keywords from the file 
    while True :

        strKeyword = wordex.readline().strip()
        if not strKeyword: 
            break
